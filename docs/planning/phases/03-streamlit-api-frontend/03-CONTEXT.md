...
- **D-05:** Streamlit should call `/api/inventory` first, then render retailer and date controls from the returned inventory payload. Sidebar control options must not be derived by scanning `InflationItems/Datas/` or calling `discover_csv_inventory()` directly from Streamlit.
