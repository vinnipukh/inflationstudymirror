...
Scope: `Codes/HousesRent/KayseriSivasTokat/` — async Python scraper for
sarı site rental listings (Kayseri, Sivas, Tokat).
...
|---|---|---|---|
| **rayobrowse** (current, v0.2.1) | Patched Chromium daemon (Docker, CDP) | **Closed-source beta** | Keep for now; risk noted in wiki (SwiftShader leak, API churn broke once). |
| **CloakBrowser** (`CloakHQ/CloakBrowser`, main @ 2026-08-15) | Stealth Chromium fork (ungoogled base), 71 claimed C++ patches | MIT wrapper, **proprietary binary** (BINARY-LICENSE v1.3; latest major = paid) | ⭐ Reference goldmine: `cloakbrowser/human/` — Bézier mouse, per-char keyboard w/ mistypes, scroll accel/decel, actionability, full parameter catalog (presets `default`/`careful`). Template for replacing our detectable `human_jittery_move()`. **Corrected 2026-08-16**: first-party site says **all plans incl. the one-session free tier run Chromium v150** (drop the "older pin" assumption — verify at install). ⚠️ "71 patches / 30/30 / 0.9 reCAPTCHA" remain **vendor marketing until G0**. Driver candidate — A/B vs the existing working driver. Does **not** resolve closed-source risk. |
...
| 8 | **browser-act** (candidate) | `browser-act/skills` (★5.4k, MIT) — `uv tool install browser-act-cli` | #1/#7/#8/#9 — stealth CLI + `solve-captcha` + `remote-assist` human handoff |
...
- [x] Site 2: CloakBrowser — evaluated, human/ template + driver candidate
