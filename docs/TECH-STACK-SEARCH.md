...
| Validation findings | Folded into `docs/APPROACH.md` and the dated status entries below; the original standalone report files are not present in this checkout |
...
| **deep-research** | `samber/deep-research` | meta | Parallel web search + multi-source validation + cited reports — **useful companion for the deep-research agent prompt** (original prompt not retained) |
...
**This blueprint was revised by the 2026-08-16 validation work.** The authoritative plan now lives in `docs/APPROACH.md` (revised): **browser-only default** (one persistent real-browser session, serial cities, index pages only), wreq demoted to evidence-gated G2b, captcha solving demoted to stop-signal + optional login fallback, snapshot-aware Mongo storage, reconciliation gates. Historical content below kept for audit trail.
...
- [x] **Deep-research validation processed 2026-08-16**; corrections were folded into `docs/APPROACH.md` and this registry: vendor/tool corrections, B0 compliance gate, B8–B12, and browser-only default. The original standalone report file is not present in this checkout.
...
## Inflation calculation code imported (2026-09-07)

Copied the calculation layer from `github.com/urazkagangunes/InflationResearchStudy`
(master @ f6a7b61) into `Inflations/Codes/` — 144 files, uncommitted (working-tree only).

What was imported:

- `Inflations/Codes/turkey_inflation.py` — Turkey-wide daily inflation calculator
  (Dutot `basic_index`, Carli `avg_inflation`, TUIK-weighted products, TUIK-weighted full;
  rent injected as group 04). Reads `_DATA_ROOT = <repo>/InflationItems/Datas`.
- `Inflations/Codes/tuik_config.py` — TUIK category/weight config used by the calculator.
- Per-store `*_inflation.py` / `tuik_config.py` scripts under 10 categories:
  ClothingStores, ConstructionSuppliesMarkets, Cosmetics, Full_Calculate, HomeGoods,
  HousesRent, Hungerthresholds, Markets, TechnologicalProducts, product-to-ctagory.
- `Inflations/Codes/Hungerthresholds/` — daily hunger-threshold calculators per market.
- `Inflations/Codes/product-to-ctagory/` — product name → TUIK CPI category mapping tooling.
- `conftest.py` at repo root (adds `Inflations/Codes` to `sys.path`).

NOT copied: the ~1.6 GB of data (`InflationItems/Datas`, `Inflations/Datas`) and no data
outputs — per user instruction. `run_all.py` / `run_all.sh` were copied then deleted at
user request (their paths reference remote-only scrapers). The clone is at
/tmp/infres/InflationResearchStudy if outputs are ever wanted.

Kept-local (already adapted to this repo's `InflationItems/Datas/` layout, differ from remote):
Inflations/Codes/ClothingStores/Vakko/{vakko_inflation,vakko_tuik_config}.py and
Inflations/Codes/Markets/Gurmar/{gurmar_inflation,gurmar_tuik_config}.py, README.md.
