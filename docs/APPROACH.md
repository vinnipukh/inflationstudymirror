...
|---|---|---|---|---|
| **B0** | **Access-policy / contractual controls** ⭐ NEW | Site terms contain AI-agent provisions: agent UA identification, no human-interaction mimicry, respect CAPTCHA/firewall/rate/robots controls; programmatic access may be blocked on breach/suspicion | **Decide the authorised access mode FIRST** (terms applicability, permission, or approved data route). Retain dated copy of terms/robots policy. Stealth/humanisation only under an appropriate authorisation model | Ask the site for an approved research/feed/data route (P2) |
| **B1** | **Cloudflare edge** — `cf_clearance` (visitor/device-bound, challenge + precursor clearance, **continuously updated**), `__cf_bm` (30-min **inactivity** expiry), `__cflb` | Fresh/unwarmed session, datacenter IP, TLS mismatch, clearance reuse | **Persistent real browser** (existing warm driver or CloakBrowser) + homepage-first warmup; session kept behaviourally alive (precursor clearance updates) | Retry warmup → rotate profile → IP change (one variable at a time) |
...
| **B9** | **Cross-stack network identity mismatch** ⭐ NEW | A non-browser HTTP client (wreq) diverges in TLS/H2/client-hints/headers/request-mode/connections; wreq predefined profiles stop at Chrome149 vs CloakBrowser 150 | **Not in default path.** If ever tested: G2b one-request identity experiment, same egress, full fingerprint capture, cookie-mutation comparison | Same-browser "middle layer" (same-origin fetch inside the trusted browser) before any external client |
...
```
        ...
        │  first; CloakBrowser/Scrapling only if G0 shows value)         │
...
| Layer | Tool | Why |
|---|---|---|
| Driver | **Existing warm driver first** (rayobrowse-style); CloakBrowser (v150) / Scrapling StealthyFetcher only if G0 shows value | No new evidence picks a winner; the existing warm session already succeeded in recon |
...
1. **G0 — Authorisation + browser baseline, then driver shootout.** Record applicable terms/robots/access permission (B0) FIRST. Benchmark the **existing working driver first**; CloakBrowser and Scrapling only if switching has value. Warm AND clean profiles. Metrics: HTTP outcome, unexpected login/challenge rate, `_px3` issuance/mutation, extraction correctness, startup time, median latency, profile recovery, total runtime. **No third-party CAPTCHA solving in this gate.**
...
| CloakBrowser vendor lock-in (B11) | Medium | Medium | Free tier v150 verified at install; OSS exit ramp (Scrapling/Camoufox) |
...
| Tool claims unverified (CloakBrowser 71 patches, NopeCHA PX) | Medium | Medium | G0/G2b benchmarks; treat vendor claims as marketing until proven |
