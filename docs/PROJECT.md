...
- **Adapters**: CSV repository for existing `InflationItems/Datas/` files first; database/cache can be added later behind the same ports.
...
- Scrapers under `InflationItems/Codes/` write raw daily CSVs into `InflationItems/Datas/`.
- Inflation calculators under `Inflations/Codes/` write processed outputs into `Inflations/Datas/`.
- `streamlit_app.py` currently exists and reads CSVs directly from `InflationItems/Datas/`.
...
- ✓ Raw scraped data exists under `InflationItems/Datas/` — existing.
- ✓ Multiple retailer/domain scrapers exist under `InflationItems/Codes/` — existing.
...
- ✓ CSV adapter preserves bounded retailer/date/max-files loading for existing `InflationItems/Datas/` files — validated in Phase 1.
