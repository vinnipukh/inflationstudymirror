...
- uses a visible, ordinary Chrome session by default;
...
and a dated checkpoint under its scraper directory. The output columns are
public listing data only. The first three columns intentionally match the
...
The default delay is two seconds between listing pages. Keep the crawl serial,
respect the site's robots/terms, and use `--max-pages-per-scope` or a narrower
geographic start URL for controlled verification.
...
from the GitHub Actions interface. The workflow runs Chrome under Xvfb, writes
`Datas/HousesRent/Emlakjet/*.csv`, and commits changes with the repository's
GitHub Actions bot account. A CAPTCHA or anti-bot challenge causes the run to
stop rather than attempting an automated bypass.

## Tests

Fixture-driven parser and pagination tests use the standard library and do not
visit the site:
