...
def csvSaver(file, timeParam, month, day):
    sum_daily_name = f"InflationData\SummaryData\Daily\\SummaryDailyTechInflation.csv"
    file_exists = os.path.isfile(sum_daily_name)
    daily_header = not file_exists or os.path.getsize(sum_daily_name) == 0

    sum_monthly_name = f"InflationData\SummaryData\Monthly\\SummaryMonthlyTechInflation.csv"
    ...
    sum_weekly_name = f"InflationData\SummaryData\Weekly\\SummaryWeeklyTechInflation.csv"
    ...
    if timeParam == "Daily":
        file.to_csv(f"InflationData\DetailedInflationData\Daily\\DetailedDailyTechInflation{month}-{day}.csv",
                    index=False, encoding="utf-8")
        newFile.to_csv(sum_daily_name, index=False, mode="a", encoding="utf-8", header=daily_header)
    if timeParam == "Weekly":
        file.to_csv(f"InflationData\DetailedInflationData\Weekly\\DetailedWeeklyTechInflation{month}-{day}.csv",
                    index=False, encoding="utf-8")
        newFile.to_csv(sum_weekly_name, index=False, mode="a", encoding="utf-8", header=weekly_header)
    if timeParam == "Monthly":
        file.to_csv(f"InflationData\DetailedInflationData\Monthly\\DetailedMonthlyTechInflation{month}-{day}.csv",
                    index=False, encoding="utf-8")
...
def checkDate(monthNum, dayNum):
    ...
    try:
        if dayNum > 7:
            test_4 = fileInput(f"Datas\\technologyProducts{str(monthNum)}-{str(dayNum)}-.csv",
                               f"Datas\\technologyProducts{str(monthNum)}-{str(dayNum - 7)}-.csv")
            csvSaver(test_4, "Weekly", monthNum, dayNum)
        else:
            if (monthNum % 2 == 0 and monthNum <= 7) or (monthNum % 2 == 1 and monthNum > 7) or (monthNum == 8):
                test_4 = fileInput(f"Datas\\technologyProducts{str(monthNum)}-{str(dayNum)}-.csv",
                                   f"Datas\\technologyProducts{str(monthNum - 1)}-{str(24 + monthNum)}-.csv")
                csvSaver(test_4, "Weekly", monthNum, dayNum)
                isExtra = True
            else:
                test_4 = fileInput(f"Datas\\technologyProducts{str(monthNum)}-{str(dayNum)}-.csv",
                                   f"Datas\\technologyProducts{str(monthNum - 1)}-{str(23 + monthNum)}-.csv")
                csvSaver(test_4, "Weekly", monthNum, dayNum)
    ...
    try:
        if dayNum > 1:
            test_4 = fileInput(f"Datas\\technologyProducts{str(monthNum)}-{str(dayNum)}-.csv",
                               f"Datas\\technologyProducts{str(monthNum)}-{str(dayNum - 1)}-.csv")
            csvSaver(test_4, "Daily", monthNum, dayNum)
        else:
            if isExtra:
                test_4 = fileInput(f"Datas\\technologyProducts{str(monthNum)}-{str(dayNum)}-.csv",
                                   f"Datas\\technologyProducts{str(monthNum - 1)}-{str(31)}-.csv")
                csvSaver(test_4, "Daily", monthNum, dayNum)
            else:
                test_4 = fileInput(f"Datas\\technologyProducts{str(monthNum)}-{str(dayNum)}-.csv",
                                   f"Datas\\technologyProducts{str(monthNum - 1)}-{str(30)}-.csv")
                csvSaver(test_4, "Daily", monthNum, dayNum)
    except Exception as e:
        print(e)
    try:
        if dayNum > 28:
            test_4 = fileInput(f"Datas\\technologyProducts{str(monthNum)}-{str(dayNum)}-.csv",
                               f"Datas\\technologyProducts{str(monthNum)}-{str(dayNum - 28)}-.csv")
            csvSaver(test_4, "Monthly", monthNum, dayNum)
        else:
            test_4 = fileInput(f"Datas\\technologyProducts{str(monthNum)}-{str(dayNum)}-.csv",
                               f"Datas\\technologyProducts{str(monthNum - 1)}-{str(dayNum)}-.csv")
            csvSaver(test_4, "Monthly", monthNum, dayNum)
    except Exception as e:
        print(e)
