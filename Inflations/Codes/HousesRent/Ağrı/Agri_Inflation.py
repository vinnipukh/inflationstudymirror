...
def load_agri_data(data_dir):
    all_files = glob.glob(os.path.join(data_dir, "*_Dailyrents.csv"))

    print(f"🔍 Found {len(all_files)} Ağrı files in:")
    ...
    if not df_list:
        raise ValueError("No valid Ağrı data found.")
...
def generate_agri_inflation_reports(input_dir, output_dir):
    print("=" * 60)
    print("📈 AĞRI RENT INFLATION ANALYSIS STARTED")
    print("=" * 60)

    try:
        df = load_agri_data(input_dir)
    except ValueError as e:
        print(f"❌ {e}")
        return

    print(f"✅ Loaded {len(df)} Ağrı rental records.")
    ...
    daily_overall.to_csv(
        os.path.join(output_dir, "Agri_Daily_Inflation_Report.csv"),
        index=False,
        encoding="utf-8-sig"
    )

    weekly_overall.to_csv(
        os.path.join(output_dir, "Agri_Weekly_Inflation_Report.csv"),
        index=False,
        encoding="utf-8-sig"
    )

    monthly_overall.to_csv(
        os.path.join(output_dir, "Agri_Monthly_Inflation_Report.csv"),
        index=False,
        encoding="utf-8-sig"
    )

    daily_rooms.to_csv(
        os.path.join(output_dir, "Agri_Daily_Room_Based_Report.csv"),
        index=False,
        encoding="utf-8-sig"
    )

    weekly_rooms.to_csv(
        os.path.join(output_dir, "Agri_Weekly_Room_Based_Report.csv"),
        index=False,
        encoding="utf-8-sig"
    )

    monthly_rooms.to_csv(
        os.path.join(output_dir, "Agri_Monthly_Room_Based_Report.csv"),
        index=False,
        encoding="utf-8-sig"
    )
...
if __name__ == "__main__":
    INPUT_DIRECTORY = r"/Users/efeyildirim/PycharmProjects/PythonProject/temizlenmis_csvler/Rents/Ağrı"

    OUTPUT_DIRECTORY = r"/Users/efeyildirim/PycharmProjects/PythonProject/inflation_reports/Rents/Ağrı"

    generate_agri_inflation_reports(
