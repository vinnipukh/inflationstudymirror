...
def load_isparta_data(data_dir):
    all_files = glob.glob(os.path.join(data_dir, "*_Dailyrents.csv"))

    print(f"🔍 Found {len(all_files)} Isparta files in:")
    ...
    if not df_list:
        raise ValueError("No valid Isparta data found.")
...
def generate_isparta_inflation_reports(input_dir, output_dir):
    print("=" * 60)
    print("📈 ISPARTA RENT INFLATION ANALYSIS STARTED")
    print("=" * 60)

    try:
        df = load_isparta_data(input_dir)
    except ValueError as e:
        print(f"❌ {e}")
        return

    print(f"✅ Loaded {len(df)} Isparta rental records.")
    ...
    daily_overall.to_csv(
        os.path.join(output_dir, "Isparta_Daily_Inflation_Report.csv"),
        index=False,
        encoding="utf-8-sig"
    )

    weekly_overall.to_csv(
        os.path.join(output_dir, "Isparta_Weekly_Inflation_Report.csv"),
        index=False,
        encoding="utf-8-sig"
    )

    monthly_overall.to_csv(
        os.path.join(output_dir, "Isparta_Monthly_Inflation_Report.csv"),
        index=False,
        encoding="utf-8-sig"
    )

    daily_rooms.to_csv(
        os.path.join(output_dir, "Isparta_Daily_Room_Based_Report.csv"),
        index=False,
        encoding="utf-8-sig"
    )

    weekly_rooms.to_csv(
        os.path.join(output_dir, "Isparta_Weekly_Room_Based_Report.csv"),
        index=False,
        encoding="utf-8-sig"
    )

    monthly_rooms.to_csv(
        os.path.join(output_dir, "Isparta_Monthly_Room_Based_Report.csv"),
        index=False,
        encoding="utf-8-sig"
    )
...
if __name__ == "__main__":
    INPUT_DIRECTORY = r"/Users/efeyildirim/PycharmProjects/PythonProject/temizlenmis_csvler/Rents/Isparta"

    OUTPUT_DIRECTORY = r"/Users/efeyildirim/PycharmProjects/PythonProject/inflation_reports/Rents/Isparta"

    generate_isparta_inflation_reports(
