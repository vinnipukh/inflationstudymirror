...
def load_igdir_data(data_dir):
    all_files = glob.glob(os.path.join(data_dir, "*_Dailyrents.csv"))

    print(f"🔍 Found {len(all_files)} Iğdır files in:")
    ...
    if not df_list:
        raise ValueError("No valid Iğdır data found.")
...
def generate_igdir_inflation_reports(input_dir, output_dir):
    print("=" * 60)
    print("📈 IĞDIR RENT INFLATION ANALYSIS STARTED")
    print("=" * 60)

    try:
        df = load_igdir_data(input_dir)
    except ValueError as e:
        print(f"❌ {e}")
        return

    print(f"✅ Loaded {len(df)} Iğdır rental records.")
    ...
    daily_overall.to_csv(
        os.path.join(output_dir, "Igdir_Daily_Inflation_Report.csv"),
        index=False,
        encoding="utf-8-sig"
    )

    weekly_overall.to_csv(
        os.path.join(output_dir, "Igdir_Weekly_Inflation_Report.csv"),
        index=False,
        encoding="utf-8-sig"
    )

    monthly_overall.to_csv(
        os.path.join(output_dir, "Igdir_Monthly_Inflation_Report.csv"),
        index=False,
        encoding="utf-8-sig"
    )

    daily_rooms.to_csv(
        os.path.join(output_dir, "Igdir_Daily_Room_Based_Report.csv"),
        index=False,
        encoding="utf-8-sig"
    )

    weekly_rooms.to_csv(
        os.path.join(output_dir, "Igdir_Weekly_Room_Based_Report.csv"),
        index=False,
        encoding="utf-8-sig"
    )

    monthly_rooms.to_csv(
        os.path.join(output_dir, "Igdir_Monthly_Room_Based_Report.csv"),
        index=False,
        encoding="utf-8-sig"
    )
...
if __name__ == "__main__":
    INPUT_DIRECTORY = r"/Users/efeyildirim/PycharmProjects/PythonProject/temizlenmis_csvler/Rents/Iğdır"

    OUTPUT_DIRECTORY = r"/Users/efeyildirim/PycharmProjects/PythonProject/inflation_reports/Rents/Iğdır"

    generate_igdir_inflation_reports(
