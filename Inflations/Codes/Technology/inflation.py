...
DATA_DIR = _PROJECT_ROOT / "InflationItems" / "Datas" / "Technology"
INFLATION_OUT_DIR = _PROJECT_ROOT / "Inflations" / "Datas" / "Technology"

logger = logging.getLogger(__name__)


def _load_csv(date_str: str):
    fpath = DATA_DIR / f"beymen_tech_{date_str}.csv"
    if not fpath.exists():
...
def _compute_metrics(df_current: pd.DataFrame, df_past: pd.DataFrame):
    ...
    merged["basic_inflation"] = (
        (merged["product-price"] - merged["past_price"]) / merged["past_price"]
    ) * 100
    ...
    valid = merged.dropna(subset=["product-price", "past_price"])
    sum_current = valid["product-price"].sum()
...
def calculate_inflation(target_date=None, compare_date=None):
    ...
    # ── Save detailed data ───────────────────────────────────────────────────
    detail_file = INFLATION_OUT_DIR / f"beymen_tech_inflation_{today_str}.csv"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Beymen Tech inflation calculator")
    parser.add_argument(
        "--date",
